...
ask_do_tutorial(void)
{
    ...
    if (!opt_set_in_config[opt_tutorial]) {
        ...
        Snprintf(buf, sizeof buf,
                 "Pon \"OPTIONS=!tutorial\" en %s para omitir esta pregunta.",
                 (rc && *rc && !norc) ? rc : "tu archivo de configuración");
        do {
            ...
            add_menu(win, &nul_glyphinfo, &any, any.a_char, 0,
                     ATR_NONE, NO_COLOR,
                     "Sí, hacer el tutorial", MENU_ITEMFLAGS_NONE);
            any.a_char = 'n';
            add_menu(win, &nul_glyphinfo, &any, any.a_char, 0,
                     ATR_NONE, NO_COLOR,
                     "No, empezar a jugar", MENU_ITEMFLAGS_NONE);

            add_menu_str(win, "");
            add_menu_str(win, buf);
            if (pass++) /* we'll get here after <space> or <return> */
                add_menu_str(win, "(Por favor elegí 'y' o 'n'.)");

            end_menu(win, "¿Querés un tutorial?");
